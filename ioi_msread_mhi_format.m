function IOI = ioi_msread_mhi_format(IOI,job)
% Reads and converts MHI single wavelength data into NIfTI format
...
try
    ...
    str_color_default = [str_red]; % Red only -- this is the default at time of recording
    ...
    %to find images):
    str_color = [str_red]; %red, green, yellow and laser speckle -- this is the desired color order for processed images
    hasRGY = [str_red];
        ...
        for s1=1:length(dirs)
            if all_sessions || sum(s1==selected_sessions)
                ...
                try
                    ...
                    % Save repetition time (fs = 2Hz for TIF format)
                    IOI.dev.TR = 1/2;
                    ...
                    % Save repetition time (fs = 2Hz for MHI single wavelength format)
                    IOI.dev.TR = 1/2;
                    IOI = disp_msg(IOI,['Problem with colors in info.txt for session ' int2str(s1) '; TR set to 0.5 and  defaulting to ' str_color_default]);    
                end
                %Read electro
...
%                     pourcent = 1;
                    dirStruct = dir(fullfile(dirs{s1},'*.tif')); % EGC
                    for iFiles = 1:numel(dirStruct),
                        fileName = fullfile(dirs{s1}, dirStruct(iFiles).name);
                        Imout = rot90(imread(fileName, 'tif'));
                        [nPixX, nPixY] = size(Imout);
                        %fname = 'my_file_with_lots_of_images.tif';
                        info = imfinfo(fileName);
                        num_images = numel(info);
                        Imout = zeros([nPixX, nPixY, 1, num_images]);
                        image_anat = squeeze(Imout(:,:,1,1));
                        for k = 1:num_images
                            Imout(:,:,1,k) = rot90(imread(fileName, k, 'Info', info));
                        end
                        % First image is anatomical
                        image_anat = squeeze(Imout(:,:,1,1));
                    end
                    %%
                    % Write one file
                    ...
                    if shrinkage_choice
                        ...
                        Imout = ioi_imresize(Imout,1,nx,ny,vx(1),vx(2));
                    end
                    ...
                    for c1=1:nColors
                        ...
                        if ~(str_color(c1)==str_laser)
                            ...
                            try
                                ...
                                % Had to typecast image_total as double //EGC
                                tenthpctle_image = prctile(double(im_obj.Data.image_total(:,:,:,:,c1)),10,4);
                                ninetiethpctle_image = prctile(double(im_obj.Data.image_total(:,:,:,:,c1)),90,4);

...
%% Load data
clear; clc
dataFolder = 'D:\Edgar\OIS_Results\lateralizationNoVis';
...
for iLPS = 1:nLPS,
    ...
    myIdx = 1;
    for iR = 3:2:numel(IOI.ROIname),
        LPS.HbO.LL(fix(myIdx/2)+1) = mean(HbOMat(iR+1,4:2:10));
        LPS.HbO.LR(fix(myIdx/2)+1) = mean(HbOMat(iR+1,3:2:9));
        LPS.HbO.RL(fix(myIdx/2)+1) = mean(HbOMat(iR,4:2:10));
        LPS.HbO.RR(fix(myIdx/2)+1) = mean(HbOMat(iR,3:2:9));
        
        LPS.HbR.LL(fix(myIdx/2)+1) = mean(HbRMat(iR+1,4:2:10));
        LPS.HbR.LR(fix(myIdx/2)+1) = mean(HbRMat(iR+1,3:2:9));
        LPS.HbR.RL(fix(myIdx/2)+1) = mean(HbRMat(iR,4:2:10));
        LPS.HbR.RR(fix(myIdx/2)+1) = mean(HbRMat(iR,3:2:9));
        
        fcliLPS.HbO(fix(myIdx/2)+1, iLPS) = ioi_lateralization_idx (LPS.HbO.LL(fix(myIdx/2)+1),...
            LPS.HbO.LR(fix(myIdx/2)+1), LPS.HbO.RL(fix(myIdx/2)+1), LPS.HbO.RR(fix(myIdx/2)+1));
        fcliLPS.HbR(fix(myIdx/2)+1, iLPS) = ioi_lateralization_idx (LPS.HbR.LL(fix(myIdx/2)+1),...
            LPS.HbR.LR(fix(myIdx/2)+1), LPS.HbR.RL(fix(myIdx/2)+1), LPS.HbR.RR(fix(myIdx/2)+1));
        myIdx = myIdx + 2;
    end
end
...
for iNaCl = 1:nNaCl,
    ...
    myIdx = 1;
    for iR = 3:2:numel(IOI.ROIname),
        NaCl.HbO.LL(fix(myIdx/2)+1) = mean(HbOMat(iR+1,4:2:10));
        NaCl.HbO.LR(fix(myIdx/2)+1) = mean(HbOMat(iR+1,3:2:9));
        NaCl.HbO.RL(fix(myIdx/2)+1) = mean(HbOMat(iR,4:2:10));
        NaCl.HbO.RR(fix(myIdx/2)+1) = mean(HbOMat(iR,3:2:9));
        
        NaCl.HbR.LL(fix(myIdx/2)+1) = mean(HbRMat(iR+1,4:2:10));
        NaCl.HbR.LR(fix(myIdx/2)+1) = mean(HbRMat(iR+1,3:2:9));
        NaCl.HbR.RL(fix(myIdx/2)+1) = mean(HbRMat(iR,4:2:10));
        NaCl.HbR.RR(fix(myIdx/2)+1) = mean(HbRMat(iR,3:2:9));
        
        fcliNaCl.HbO(fix(myIdx/2)+1, iNaCl) = ioi_lateralization_idx (NaCl.HbO.LL(fix(myIdx/2)+1),...
            NaCl.HbO.LR(fix(myIdx/2)+1), NaCl.HbO.RL(fix(myIdx/2)+1), NaCl.HbO.RR(fix(myIdx/2)+1));
        fcliNaCl.HbR(fix(myIdx/2)+1, iNaCl) = ioi_lateralization_idx (NaCl.HbR.LL(fix(myIdx/2)+1),...
            NaCl.HbR.LR(fix(myIdx/2)+1), NaCl.HbR.RL(fix(myIdx/2)+1), NaCl.HbR.RR(fix(myIdx/2)+1));
        myIdx = myIdx + 2;
    end
end
...
myIdx = 1;
for iR = 1:2:8,
    [p.HbO(fix(myIdx/2)+1), h.HbO(fix(myIdx/2)+1)] = ranksum(fcliNaCl.HbO(fix(iR/2)+1, :), ...
        fcliLPS.HbO(fix(iR/2)+1, :));
    [p.HbR(fix(myIdx/2)+1), h.HbR(fix(myIdx/2)+1)] = ranksum(fcliNaCl.HbR(fix(iR/2)+1, :), ...
...
myIdx = 1;
for iR = 1:2:8,
    dataPoints{myIdx} = fcliNaCl.HbO(fix(iR/2)+1, :);
    dataPoints{myIdx+1} = fcliLPS.HbO(fix(iR/2)+1, :);
    myIdx = myIdx + 2;
end
...
set(gca, 'XTick',3.5:2:20)       
set(gca, 'XTickLabel',{'C' 'S' 'R' 'M'},...
    'FontSize',14)
...
for iR = 1:2:8,
    dataPints{iR} = fcliNaCl.HbR(fix(iR/2)+1, :);

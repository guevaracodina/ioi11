function [U,S,V,flag] = ioi_svds(varargin)
...
% B is memory mapped file to avoid out of memory errors
% create large memmapfile for B(several GB)
fmemB_name = 'svdB.dat';
fid = fopen(fmemB_name,'w');
fwrite(fid, [sparse(m,m) A; A' sparse(n,n)], 'double');
fclose(fid);
im_objB = memmapfile(fmemB_name,'format',...
    {'double' [m+n m+n] 'svdB'},...
    'writable',true);
...
[W,D,bflag] = eigs(im_objB.data.svdB,bk,bsigma,boptions);

function SCKS = ioi_SCKS(SCKS,fig)
...
%--------------------------------------------------------------------------
sR      = cell(1,T);
[sR{:}] = deal(sparse(real(spm_sqrtm(inv(M(1).V)))*sqrt(dt)));   % observation noise variance
...
for run = 1:RUN
    ...
    for t = 2:T
        ...
        if ~isempty(M(1).Q) && iter~=1
            %Not clear if should keep the 4 lines below?
        ...
        yPred0     = M(1).g(xPred(xind,:),xPred(uind,:),pE,M(1));
        %------------------------------------------------------------------
        % UPDATE STEP:
        %------------------------------------------------------------------
        ...
        for it = 1:iter
            ...
            if ~isempty(M(1).Q) && iter~=1
                sR{t} = real(spm_sqrtm(1/(NU-no-1)*V));
            ...
            yPred = yPred0 + sR{t-1}*CubPtArray(end-no+1:end,:); %?
            y1    = sum(yPred,2)/nPts;
...
% %         
        ...
        if RUN>1 && (~isempty(ip) || ~isempty(M(1).Q))
            ...
            if ( ( (abs(MLdiff(run)/MLdiff0)<Itol && iter0(run+1) == 1) || ...
                    run==RUN) || (isempty(ip)&&MLdiff(run)<Itol) || ...
                    EXEC ),
            ...
            for i = 1:T
                ...
                if ~isempty(ip)
                    SCKS.qP.C{1}(:,i) = diag(SSb{i}(wind,wind));
                    SCKS.qP.C{2}(:,i) = diag(SSf{i}(wind,wind));
                end

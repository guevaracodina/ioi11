function [corrMatrixRaw corrMatrixRawFname] = ioi_roi_corr_raw(job,SubjIdx)
                        ...
                        if ~isempty(ROI{r1}{s1,c1})
                            if job.generate_figures
                                ...
                                if job.save_figures
                                    ...
                                    if isfield(job.optFig, 'figSize') && isfield(job.optFig, 'figRes')
                                        % Save as PNG
                                        print(h, '-dpng', fullfile(dir_ioimat,newName), sprintf('-r%d',job.optFig.figRes));
                                    else
                                        % Save as PNG

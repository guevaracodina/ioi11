...
%  de la figure 8a) est ecrit en mm-1, mais les vraies unités sont des mm
...
Ext.Dunn=[ 480 0.52     %extrapolé Simon
        560 0.52    %Données de Dunn2005
        570 0.5     %Données de Dunn2005
        580 0.5     %Données de Dunn2005
        590 .93     %Données de Dunn2005
        600 1.58    %Données de Dunn2005
        610 2.05    %Données de Dunn2005
        620 2.7     %extrapolé à l'aide de Kohl
        630 3.1     %extrapolé à l'aide de Kohl
        640 3.64    %extrapolé à l'aide de Kohl
        650 3.85    %extrapolé à l'aide de Kohl
        750 3.85];  %extrapolé Simon
...
xi = linspace(L1,L2,N);
x=E(:,1); % avant x = linspace(E(1,1),E(end,1),size(E,1)); %remplacé fev 09
pathlength = E(:,2);
pathlength = interp1(x,pathlength,xi);

% trouve L pour des concentration différentes de 0.02 (à partir de la
% figure 8 de kohl). On multiplie la courbe à 0.02 par un facteur de
% correction   
x=[0.01 0.02 0.04 0.06 0.08 0.10 0.14 0.18]; %concentration d'hémoglobine
y=[  .33 .21 .115 .09 .075 0.062 .05 .04 ]; % valeur Da=[OD*mm] à 480 nm

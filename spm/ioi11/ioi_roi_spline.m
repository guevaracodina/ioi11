...
%           natural cubic spline, 'cardinal' for cardinal cubic spline). 
%           DEFAULT = 'natural'
...
for j=3:nargin
    ...
    if ~isempty(arg)
        if ischar(arg)
            ...
            [ignore,d] = size(arg);
            if ignore~=1
                error('SPLINES:FNPLT:wrongarg',['arg',num2str(j),' is incorrect.']);
            end
            if d==1
                linewidth = arg;
            else
...
if nargout == 0
    if iscell(x)
        ...
        switch d
            case 1
                plot(x,v,symbol,'linew',linewidth);
            case 2
                plot(v(1,:),v(2,:),symbol,'linew',linewidth);
            otherwise

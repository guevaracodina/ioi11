...
x(2:end) = exp(x(2:end)); 
  [ a P1 P]=IOI_extraitVar(M,P,x);
 

if isfield(M,'source2')   % no simulation of flow
    fv       = x(3)^(1/P(4));
    ff       = (1 - (1 - P(5))^(1/(u(1)+1)))/P(5);
    f(1)     = 0;%   
    f(2)     = 0;%a.effFlow*u(1); %
    f(3)     = ((u(1)+1) - fv)/(P(3)*x(3));
    f(4)     = (ff*(u(1)+1) - fv*x(4)/x(3))/(P(3)*x(4));
    return
end
% Fout = f(v) - outflow
%--------------------------------------------------------------------------

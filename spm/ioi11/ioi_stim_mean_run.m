    ...
    try
        ...
        if ~isfield(IOI.res,'seriesOK')
            ...
            if ~isfield(IOI.res,'meanOK') || job.force_redo
                ...
                Dmb = cell(Nroi,maxM);
                Rs = cell(Nroi,maxM);
%                 whole_response = cell(length(IOI.sess_res),1);
%                 whole_response1 = cell(Nroi,4);          
                            ...
                            if all_ROIs || sum(r1==selected_ROIs)
                                ...
                                for s1=1:length(IOI.sess_res)
                                    ...
                                    if all_sessions || sum(s1==selected_sessions)
                                        ...
                                        %****by Cong on 2012/12/07  
                                        %filter the noise in the original figure 
%                                         non_average_response = 1;
%                                         if (non_average_response == 1) && (c1>4)                                           
%                                             tmp_d2 = [];
%                                             tmp_d2 = tmp_d;
%                                             HPF.hpf_butter_On = 1;
...
%                                                     tmp_DC = median(tmp_d2);
%                                                     tmp_d2 = ButterHPF(1/IOI.dev.TR,HPF.hpf_butter_freq,HPF.hpf_butter_order,tmp_d2);
%                                                     tmp_d2 = tmp_d2+ tmp_DC; %add back the DC component
%                                             end                                     
%                                             if lpf_hemoglobin_butter_On
%                                                    tmp_d2 = ButterLPF(1/IOI.dev.TR,cutoff,FilterOrder,tmp_d2);                                                
%                                             end
%                                             whole_response1{r1,c1-4} = tmp_d2; 
%                                             
%                                         end
%                                         whole_response{s1,1} = whole_response1;
%                                         %****end
...
%                                                         all_stimulations = 1;
%                                                         if all_stimulations 
%                                                             for i=1:length(Sb{r2,m1}{c1,s1}(:,1))
%                                                                 n=figure
%                                                             end
%                                                         end
...
%                                 for s1=1:length(IOI.sess_res)                                   
%                                     if all_sessions || sum(s1==selected_sessions)                                                                               
%                                         HbO = whole_response{s1}{r1,1};
%                                         HbR = whole_response{s1}{r1,2};
%                                         Flow = whole_response{s1}{r1,3};
%                                         HbT = whole_response{s1}{r1,4};
%                                         n = figure;
%                                         ls = linspace(0,length(HbO)*IOI.dev.TR,length(HbO));
%                                         plot(ls,HbO,'r');hold on;  %****************plot HbO
%                                         plot(ls,HbR,'b'); hold on; %plot HbR
%                                         plot(ls,Flow,'k'); hold on; %**plot flow
%                                         plot(ls,HbT,'g'); hold on; %*plot HbT
%                                         onsets = onsets_list{s1}{m1};
%                                         for i = 1: length(onsets)
%                                             plot([onsets(i),onsets(i)],[min(HbR),max(HbO)],'c');hold on;
%                                         end
%                                         xlabel('time(s)');
%                                         ylabel('percent changes');
%                                         %         ylabel('percent change');
%                                         tit = ['Session ' int2str(s1) ', ROI ' IOI.ROIname{r1}];
%                                         title(tit);
%                                         %         legend('HbO','HbR','HbT','onsets','the end of seizures');
%                                         legend('HbO','HbR','Flow','HbT','stimulation');
%                                         if save_figures
%                                             if isfield(IOI.res.ROI{r1},'name')
%                                                 tit = [IOI.subj_name ' ROI ' int2str(r1) ' ' IOI.res.ROI{r1}.name ', Session ' int2str(s1) ', whole hemoglobin changes'];
%                                             else
%                                                 tit = [IOI.subj_name ' ROI ' int2str(r1) ' ' int2str(r1) ', Session ' int2str(s1) ', whole hemoglobin changes'];
%                                             end
%                                             title(tit);
%                                             ioi_save_figures(save_figures,generate_figures,n,tit,dir_fig);
%                                         end
%                                         close(n);
%                                     end
%                                 end
                                        ...
                                        for m1=1:length(onsets_list{s1})
                                            if any(m1==job.which_onset_type)
                                                ...
                                                for r1=1:length(ROI)
                                                    if all_ROIs || sum(r1==selected_ROIs)
                                                        r2 = r2+1;
                                                        ColorSet = varycolor(length(Sa{r2,m1}{c1,s1}(:,1)));
                                                        for c1 = ctotal
                                                            h1 = h1 + 1;
                                                            h(h1) = figure;
                                                            set(gca, 'ColorOrder', ColorSet);
                                                            hold all
                                                            for i=1:length(Sa{r2,m1}{c1,s1}(:,1))
                                                                plot(ls,Sa{r2,m1}{c1,s1}(i,:)); 
                                                            end
                                                            legend off
                                                            set(gcf, 'Colormap', ColorSet);
                                                            hc1 = ioi_set_colorbar(gcf,length(Sa{r2,m1}{c1,s1}(:,1)));
                                                            if save_figures
                                                                if isfield(IOI.res.ROI{r1},'name')
                                                                    tit = [IOI.subj_name ' ROI ' int2str(r1) ' ' IOI.res.ROI{r1}.name ', Session ' int2str(s1) ', Color ' IOI.color.eng(c1)];
                                                                else
                                                                    tit = [IOI.subj_name ' ROI ' int2str(r1) ' ' int2str(r1) ', Session ' int2str(s1) ', Color ' IOI.color.eng(c1)];
                                                                end
                                                                title(tit);
                                                                ioi_save_figures(save_figures,generate_figures,h(h1),tit,dir_fig);
                                                            end

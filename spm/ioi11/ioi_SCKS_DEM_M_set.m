function [M] = ioi_SCKS_DEM_M_set(M)
...
for i = 1:g
    ...
    %----------------------------------------------------------------------
    np  = length(spm_vec(M(i).PS.pE));
 
    % Assume fixed parameters if not specified
    %----------------------------------------------------------------------
    if isempty(M(i).PS.pC)
        M(i).PS.pC = sparse(np,np);
    end
 
    % convert variances to covariances if necessary
    %----------------------------------------------------------------------
    if isvector(M(i).PS.pC)
        M(i).PS.pC = sparse(diag(M(i).PS.pC));
    end
    
    % convert variance to covariances if necessary
    %----------------------------------------------------------------------
    if isscalar(M(i).PS.pC)
        M(i).PS.pC = speye(np,np)*M(i).PS.pC;
    end
 
    % check size
    %----------------------------------------------------------------------
    if length(M(i).PS.pC) ~= np
        warndlg(sprintf('please check: M(%i).PS.pC',i))
    end
